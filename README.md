...
- `dev-logs-all.bat` - просмотр всех логов (backend + frontend)
...
- **Frontend логи** - DevLogViewer в UI (кнопка 🐛)
  ...
  - 📁 Файлы: `backend/logs/frontend/frontend-YYYY-MM-DD.log`
...
- **Frontend логи**:
  - Локальная разработка: `backend/logs/frontend/frontend-YYYY-MM-DD.log`
  - Docker: `/var/log/app/frontend/frontend-YYYY-MM-DD.log`
- **Автоочистка**: логи старше 7 дней удаляются автоматически
- **Ротация**: новый файл каждый день

### 🔍 Просмотр логов
```bash
# Все логи (backend + frontend) - интерактивно
dev-logs-all.bat

# Только backend логи - интерактивно
dev-logs-simple.bat

# Расширенный просмотр backend логов (с jq)
dev-logs.bat

# Последние 20 строк backend логов
powershell "Get-Content backend/logs/app-*.log -Tail 20"

# Последние 20 строк frontend логов
powershell "Get-Content backend/logs/frontend/frontend-*.log -Tail 20"

# Поиск по всем логам
findstr "ERROR" backend/logs/app-*.log backend/logs/frontend/frontend-*.log
```
